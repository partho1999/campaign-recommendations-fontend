...
export default function Page() {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/predict-date-range/?start_date=${startDate}&end_date=${endDate}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch("http://127.0.0.1:8000/api/prediction-daily");
