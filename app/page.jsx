...
export default function AdRecDashboard() {
  ...
  useEffect(() => {
    const fetchPredictionData = async () => {
      setLoading(true)
      try {
        const url = `https://campaign-recommendations-backend.onrender.com/api/prediction-run?hours_back=24`
  ...
  const totalCost = recommendations.reduce((sum, r) => sum + r.metrics.cost, 0)
  const totalRevenue = recommendations.reduce((sum, r) => sum + r.metrics.revenue, 0)
  const totalProfit = recommendations.reduce((sum, r) => sum + r.metrics.profit, 0)
  const totalClicks = recommendations.reduce((sum, r) => sum + r.metrics.clicks, 0)
          ...
          <>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Analysis for {currentData.range} ({summary?.date_range.from} to {summary?.date_range.to})
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {summary?.recommendation_counts &&
                    Object.entries(summary.recommendation_counts).map(([rec, count]) => (
                      <div key={rec} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-2">
                          {getRecommendationIcon(rec)}
                          <span className="font-medium">{rec}</span>
                        </div>
                        <Badge variant={getRecommendationColor(rec)}>{count}</Badge>
                      </div>
                    ))}
                </div>
              </CardContent>
